...
let error(s: string) = failwith (Printf.sprintf "SIMPLE_PROD_SL_DOMAIN_ERROR: %s" s)

module MAKE_SIMPLE_PROD_SL_DOMAIN =
  functor (L: SL_DOMAIN) -> 
    functor (R: SL_DOMAIN) -> 
      functor (O: OPTION) -> 
(struct
   ...
   let debug = O.debug
   let name = "SIMPLE_PROD_SL_DOMAIN"
...
(*	if List.length lefts = 1 && List.length rights = 1 then
    ...
    let pp: t -> unit = fun t -> 
      O.XML.print_center "SIMPLE_PROD_SL_DOMAIN";
      O.XML.printf "<div>\n<span class='dp_i'>\n";
      L.pp t.left;
      O.XML.printf "</span>\n<span class='dp_i'>";
      R.pp t.right;
